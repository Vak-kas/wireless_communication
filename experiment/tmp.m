...
retrans = 0; % 전체 재전송 횟수
count = 0 ;%재전송 3회 제한을 위한 변수



h_index = 1; % h (무선채널) 인덱스
data_index = 1; % 데이터 인덱스 
flag = flase; % 재전송 중인 상태인가?

while true
    %재전송 상태가 아닌 순수한 상태일 때
    if flag == true
...
% function [] = func(bit_data)
%     global drop_count,retrans, h, noise, h_index, transmit_power;
% 
%     if length(bit_data) == 0
%         return
%     end
%     flag = -1;
%     for i = 1:3
% 
%         if length(bit_data) == 4
%             data = QAM_bit2data(bit_data);
%             modulated_symbol = BPSK_Mapping(data);
%         elseif length(bit_data) == 2
%             data = QPSK_bit2data(bit_data);
%             modulated_symbol = QPSK_Mapping(data);
%         elseif length(bit_data) ==1
