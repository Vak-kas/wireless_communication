...
M = 4; % BPSK =2, QPSK = 4, 8-Psk = 8 중에서 선택 
nSymbol = 100000; % 각 상황 테스트 비트 개수

Repeat_time = 3; %FEC를 위해서 몇 번을 반복할 것인지
print_SER_graph = false; %true면 SER 그래프 그림을 그림
print_BER_graph = false; %true면 BER 그래프 그림을 그림
hod = false;
...
%     plot(real(received_symbol), imag(received_symbol), "ro");
    ...
    %% SER
    SER(k) = sum(data ~= recovered_data)/(nSymbol/log2(M));




    %% BER
    BER(k) = sum(bit_data ~= recovered_bit_data) / (nSymbol/log2(M));


end


if print_SER_graph == true
    figure(100);
    semilogy(SNR_dB, SER, "b"); grid on;
    ylim([10^-5 1]); xlim([0 20]);
    ylabel("SER");
    xlabel("SNR(dB)");
end

if print_BER_graph == true
    if hod == true
        hold on;
        figure(100);
