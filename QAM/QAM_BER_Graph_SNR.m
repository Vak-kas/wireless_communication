...
Repeat_time = 1:2:10; % FEC를 위한 반복 횟수, FEC를 하지 않을 거면 1로 설정, 할 거면 다른 숫자 입력
method = 2;
...
for k = 1:length(Repeat_time)
    ...
    data = zeros(1, nSymbol/log2(M)*Repeat_time(k));
    
    %% setData
    if M==16
        for i = 1:nSymbol/log2(M)*Repeat_time(k)
    ...
    noise = (randn(1, nSymbol/log2(M)*Repeat_time(k)) +1j*randn(1, nSymbol/log2(M)*Repeat_time(k)) ) .* noise_power;
    ...
    xlim([-2, 2]);
    ylim([-2, 2]);
    plot(real(received_symbol)/sqrt(10), imag(received_symbol)/sqrt(10), "ro");
...
figure(100);
semilogy(Repeat_time, BER, "b"); grid on;
ylim([10^-5 1]); xlim([0 10]);
ylabel("BER");
xlabel("Repeat_time");
